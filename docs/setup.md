...
- `AI_COMPANION_TTS_USE_PERSISTENT_APLAY`
...
`AI_COMPANION_TTS_USE_PERSISTENT_APLAY` controls whether `aplay` playback uses a long-lived persistent stream or one-shot command playback. On some Raspberry Pi HDMI paths, leaving it `false` gives cleaner speech playback.
...
2. Create `.venv` and run `python -m pip install -e ".[dev,ui]"`, or `python -m pip install -e ".[dev,tts,ui]"` if you want local Piper TTS plus the fullscreen face renderer.

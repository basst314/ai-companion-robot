...
write_env_file() {
  ...
  if [[ "${PLATFORM}" == "macos" ]]; then
    ...
    audio_command="ffmpeg -y -f avfoundation -i :${audio_device_index} -t {duration_seconds} -ar 16000 -ac 1 {output_path}"
  else
    audio_command='arecord -d {duration_seconds} -f S16_LE -r 16000 -c 1 {output_path}'
  fi
...
AI_COMPANION_RECORD_SECONDS=${DEFAULT_RECORD_SECONDS}
...
Next steps:
  ...
  2. Press Enter at the speech prompt to record a short utterance
  3. Edit ${ENV_FILE} if you want to adjust model, language mode, or recorder settings
EOF
}

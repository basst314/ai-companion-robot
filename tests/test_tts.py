...
def test_command_playback_service_warms_aplay_on_first_playback_only(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    async def run() -> None:
        service = CommandAudioPlaybackService(
            ...
            warmup_settle_ms=0,
            warmup_idle_threshold_seconds=60.0,
        )
    ...
    assert commands[2][:3] == ("aplay", "-D", "default:CARD=vc4hdmi1")


def test_command_playback_service_skips_warmup_for_non_aplay(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    commands: list[tuple[str, ...]] = []
    ...
    async def run() -> None:
        service = CommandAudioPlaybackService(
            command_template=("afplay", "{input_path}"),
            output_dir=tmp_path,
            timeout_seconds=5,
            warmup_enabled=True,
        )
